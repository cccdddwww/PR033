...
void HunterROSMessenger::SetupSubscription() {
  ...
  //driverless_cmd_subscriber_ = nh_->subscribe<pcl_test::customHunter>(
    //  "/cmd_msg", 5, &HunterROSMessenger::DriverlessCbk, this);
...
/*void HunterROSMessenger::DriverlessCbk(
    const pcl_test::customHunter::ConstPtr &msg){
  double steer_cmd = msg->steering_angle;//linear_velocity
  ...
  double phi_i = ConvertCentralAngleToInner(msg->steering_angle);
    // std::cout << "set steering angle: " << phi_i << std::endl;
  hunter_->SetMotionCommand(msg->linear_velocity, phi_i);
}*/
